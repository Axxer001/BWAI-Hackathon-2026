...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 animate-slideUp">
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div
                class="px-6 py-5 border-b border-slate-100 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                ...
                <span
                    class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border {{ $report->status === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-200' : ($report->status === 'resolved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-600 border-slate-200') }}">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                <div class="rounded-3xl overflow-hidden border border-slate-200 shadow-sm">
                    <img src="{{ asset($report->photo_url) }}" alt="Violation evidence"
