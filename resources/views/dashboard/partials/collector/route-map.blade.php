...
@section('content')
    ...
    <div class="mb-8 animate-slideUp flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500">Live navigation display for {{ $session->truck->plate_number ?? 'assigned truck' }} connecting checkpoints in {{ auth()->user()->barangay->name ?? 'your zone' }}.</p>
        </div>
                    ...
                    <div class="point-card p-4 rounded-xl border {{ $point->isCollected() ? 'bg-emerald-50/50 border-emerald-100' : ($point->isSkipped() ? 'bg-rose-50/50 border-rose-100' : 'bg-slate-50/50 border-slate-100') }} transition-all flex flex-col gap-3">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                ...
                                <h4 class="text-sm font-bold text-slate-800 leading-snug">{{ $point->garbagePoint->name ?? 'Unnamed Point' }}</h4>
                                <p class="text-xs text-slate-400 mt-0.5">{{ $point->garbagePoint->address ?? 'No address listed' }}</p>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Collect points coordinates from PHP, with robust null-checks
            const points = [];
            @foreach($sessionPoints as $point)
                @if($point->garbagePoint && $point->garbagePoint->latitude && $point->garbagePoint->longitude)
                    points.push({
                        id: "{{ $point->id }}",
            ...
            if (points.length === 0) {
                document.getElementById('map').innerHTML = '<div class="flex items-center justify-center h-full text-slate-400">No valid coordinates found for this route.</div>';
                return;
            }

            // Center map around first point
                ...
                form.addEventListener('submit', async function(e) {
                    ...
                    try {
                        ...
                        if (json.success) {
                            ...
                            if (collectBtn && status === 'collected') {
