        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200 overflow-hidden flex flex-col animate-slideUp" style="animation-delay: 0.1s;">
            @if($collectionPoint)
                ...
                <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/60">
                    ...
                    <a href="https://www.openstreetmap.org/?mlat={{ $collectionPoint->latitude }}&mlon={{ $collectionPoint->longitude }}#map=17/{{ $collectionPoint->latitude }}/{{ $collectionPoint->longitude }}" target="_blank"
                ...
                <div class="w-full flex-1 min-h-[400px] relative">
                    <iframe
                        ...
                        marginheight="0"
                        marginwidth="0"
                        src="https://www.openstreetmap.org/export/embed.html?bbox={{ $collectionPoint->longitude - 0.01 }}%2C{{ $collectionPoint->latitude - 0.01 }}%2C{{ $collectionPoint->longitude + 0.01 }}%2C{{ $collectionPoint->latitude + 0.01 }}&layer=mapnik&marker={{ $collectionPoint->latitude }}%2C{{ $collectionPoint->longitude }}"
            ...
            @else
                <div class="flex-1 flex flex-col items-center justify-center p-10 text-center">
                    ...
                    <h3 class="text-base font-bold text-slate-800">No Collection Point Assigned</h3>
                    <p class="text-sm text-slate-500 mt-2 max-w-sm">Please update your profile to select your nearest barangay collection point.</p>
                </div>
            @endif
        </div>
        ...
        <div class="lg:col-span-1 flex flex-col gap-5 animate-slideUp" style="animation-delay: 0.2s;">
            ...
            <div class="bg-slate-900 rounded-2xl p-6 text-white relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    @if($activeSession)
                        {{-- 
                            Note: Calculating exact "stops away" and ETA dynamically requires a 
                            Google Maps Distance Matrix API call or complex Haversine formulas in the controller.
                            For now, this displays the active session details.
                        --}}
                        <p class="text-slate-400 text-xs mb-1">Truck is currently on route</p>
                        <div class="flex items-end gap-2 mb-1">
                            <span class="text-3xl font-extrabold tracking-tight text-white leading-none">Ongoing</span>
                        </div>
